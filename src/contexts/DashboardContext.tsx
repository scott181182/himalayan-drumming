...
import type {
  FileEntryBasicFragment,
  LocationCompleteFragment,
  VillageInContextFragment,
  PersonInContextFragment,
} from "@/generated/graphql";
import {
  GetFullContextDocument,
  GetPartialContextDocument,
  GetPersonDocument,
  GetVillageDocument,
} from "@/generated/graphql";
...
function updateOrAdd<T extends { id: string }>(arr: T[], item: T): T[] {
  let hasItem = false;
  ...
  return hasItem ? newArr : [...arr, item];
}

// oxlint-disable-next-line max-lines-per-function
export const dashboardReducer: Reducer<DashboardContextValue, DashboardDispatchAction> = (
  state,
  action,
) => {
  switch (action.type) {
    ...
    case "updateVillage":
      return {
        ...state,
        villages: updateOrAdd(state.villages, action.payload),
      };
    case "setPeople":
      return {
        ...state,
    ...
    case "updatePerson": {
      return {
        ...state,
        people: updateOrAdd(state.people, action.payload),
      };
    }
    ...
    case "updateFile":
      return {
        ...
        selectedFiles: state.selectedFiles.map((sf) =>
          sf.id === action.payload.id ? action.payload : sf,
        ),
      };
    case "filterFiles":
      return {
...
export function useDashboardState() {
  ...
  if (!state) {
    throw new Error("Dashboard context tried to be accessed before it was initialized!");
  }
  return state;
}
    ...
    () => ({
      ...
      refetchResources: (options) => {
        return apolloClient
          ...
          .then((res) => {
            if (!res.data) {
              throw new Error("No data returned from successful refetchResources query", {
                cause: res,
              });
            }
            if (options.fileIds) {
        ...
        return apolloClient
          ...
          .then((res) => {
            if (res.data?.person) {
              ...
              console.error(JSON.stringify(res.error, null, 4));
            }
          });
      },
  ...
  return (
    <DashboardContext.Provider value={state}>
      <DashboardDispatchContext.Provider value={functions}>
        <AsyncData data={state.fileTree} loading={loading} error={error}>
          {children}
        </AsyncData>
