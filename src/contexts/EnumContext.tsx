...
export function useEnums() {
  ...
  if (!state) {
    throw new Error("Enum context tried to be accessed before it was initialized!");
  }
  return state;
}

export function EnumProvider({ children }: LayoutProps) {
  ...
  return (
    <EnumContext.Provider value={state}>
      <AsyncData data={state} loading={loading} error={error}>
        {children}
      </AsyncData>
