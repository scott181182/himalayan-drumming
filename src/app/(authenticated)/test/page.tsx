...
const personColumns: ResourceSelectorColumns<PersonForTableFragment> = [
  ...
  },
  {
    ...
    key: "children",
    render: (_, record) =>
      record.children.map((child) => <Tag key={child.id}>{child.name}</Tag>) ?? null,
  },
  {
    title: "Villages",
    key: "villages",
    render: (_, record) =>
      record.villages.map((village) => {
        const villageTag = <Tag key={village.village.id}>{village.village.name}</Tag>;
        return village.description ? (
          <Tooltip title={village.description} key={village.village.id}>
            {villageTag}
          </Tooltip>
        ) : (
          villageTag
...
const TestPage: NextPage = () => {
  ...
  const onQuery = useCallback(
    (filters: Readonly<Record<string, FilterValue | null>>, search?: string) => {
  ...
  return (
    <Row className="w-full p-4" justify="center">
      <Col span={4}>
        <ResourceSelector
          value={value}
          onChange={setValue}
