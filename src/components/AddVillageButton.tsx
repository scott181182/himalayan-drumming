...
export function AddVillageButton() {
  ...
  const [createVillageMutation] = useMutation(CreateVillageDocument, {
    onCompleted: (res) => {
      void refetchVillage(res.createVillage.id);
    },
  });
  ...
  const createVillage = useCallback(() => {
    if (!selectedLocation) {
      ...
      message.warning(
        "Could not find selected point on map. Please select another coordinate and try again.",
      );
      return;
    }
  ...
  const btn = (
    <Button onClick={openModal} disabled={!selectedLocation}>
      Add Village
    </Button>
  );
  ...
  return (
    <>
      {selectedLocation ? (
        ...
        <Tooltip title="A location must be selected to create a village">{btn}</Tooltip>
      )}
      <Modal title="Create Village" open={open} onCancel={closeModal} onOk={createVillage}>
        <Form form={form}>
          <Form.Item
            label="Name"
            name="name"
