...
export function AddPersonToVillageButton({
  village,
  onSubmit,
}: Readonly<AddPersonToVillageButtonProps>) {
      ...
      variables: {
        ...
        data: {
          people: {
            create: [
              {
                person: { connect: { id: data.person.id } },
  ...
  return (
    <>
      ...
      <Modal title="Add Person" open={open} onCancel={closeModal} onOk={addPerson}>
        <Form form={form}>
          <Form.Item
            label="Person"
            name="person"
          ...
          <Form.Item label="Relation" name="description">
            <Input />
          </Form.Item>
        </Form>
      </Modal>
