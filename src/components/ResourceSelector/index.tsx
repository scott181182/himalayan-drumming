...
export interface ResourceSelectorColumn<V> extends ResourceSelectorColumnBase<V> {
  filterOptions?:
    | Omit<ColumnFilterItem, "children">[]
    | ((values: V[]) => Omit<ColumnFilterItem, "children">[]);
}
...
function transformColumns<T>(columns: ResourceSelectorColumns<T>, data: T[]): ColumnsType<T> {
  return columns.map((colDef) => {
    ...
    return {
      ...column,
      filters: typeof filterOptions === "function" ? filterOptions(data) : filterOptions,
    };
  });
}
...
export function ResourceSelector<V extends object, QRes, QVar extends OperationVariables>({
  ...
  const selectOptions = useMemo<DefaultOptionType[]>(() => {
    ...
    // oxlint-disable-next-line typescript/no-unsafe-type-assertion
    return [{ label: renderValue(value), value: value[recordKey] as string | number }];
  ...
  const { data, loading, error } = useQuery(queryDocumentNode, {
    variables: onQuery(filterValue, searchValue),
    // This is set primarily so we don't fetch data until the user clicks on the selector.
    skip: !modalOpen,
  });
  ...
  const tableColumns = useMemo(
    () => transformColumns(columns, dataSource ?? []),
    [columns, dataSource],
  );
  ...
  const onTableChange = useCallback(
    (_pagination: TablePaginationConfig, filters: Record<string, FilterValue | null>) => {
  ...
  return (
    <>
      ...
      >
        ...
        {error && (
          <Alert type="error" description="There was an error fetching data. Please try again!" />
        )}
        <Table
          ...
          rowSelection={{
            ...
            // oxlint-disable-next-line typescript/no-unsafe-type-assertion
            selectedRowKeys: selectedValue ? [selectedValue[recordKey] as string | number] : [],
          }}
          rowKey={recordKey}
        />
      </Modal>
