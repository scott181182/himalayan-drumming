...
type MakeMarkerHandlerFn = (loc: LocationCompleteFragment) => LeafletEventHandlerFnMap;
...
export function Map() {
  ...
  const makeMarkerHandler = useCallback<MakeMarkerHandlerFn>(
    (loc) => ({
      click: (ev) => {
  ...
  const fileMarkers = useMemo(
    () =>
      locations
        ?.filter((l) => !villages.some((v) => v.location.id === l.id))
        .map((l) => (
          <Marker
            key={l.id}
  ...
  return (
    <MapContainer center={UTTARAKHAND_CENTER} zoom={9} scrollWheelZoom={true} className="h-full">
      <TileLayer attribution={FreeTileLayers.ESRI.attribution} url={FreeTileLayers.ESRI.url} />
      {/* oxlint-disable-next-line typescript/no-unsafe-type-assertion */}
      <GeoJSON data={uttarakhandGeo as GeoJSONProps["data"]} />
      {villageMarkers}
      {fileMarkers}
      <ClickMarker />
